...
public class FileUtil3 {
	// 업로드 위치->절대경로(c:\~
	public static final String UPLOAD_PATH = "C:/restaurantImg";

	// 1.원본파일의 이름을 변경할 메서드
	// ex) test.txt or test.TXT ->lastIndex(.) ,substring()이용
	public static String rename(String filename) throws Exception {
		if (filename == null)
			return null;// 업로드를 하지 않은 경우
		// 규칙정해주기->시스템의 날짜+임의의 숫자를 조합해서 새로운 파일생성
		String prefix = "\\img/";
		String newName = prefix+filename; // 0~49
	...
	public static String rename(String filename, String newName) throws Exception {
		...
		newFileName = newName + extention.toLowerCase();// 확장자가
																	// 대문자->소문자
		System.out.println(newFileName);
		return newFileName;
